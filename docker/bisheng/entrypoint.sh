...
start_workflow(){
  # 工作流相关的celery worker
    celery -A bisheng.worker.main worker -l info -c 50 -P threads -Q workflow_celery -n workflow@%h
}
...
start_default(){
    ...
    celery -A bisheng.worker.main worker -l info -c 50 -P threads -Q celery -n celery@%h
}
